...
/**
 * Created by VINH PHUC on 2/8/2018
 */
public class ArtistSongAdapter extends BaseSongAdapter<ArtistSongAdapter.ItemHolder> {
    ...
    public void onBindViewHolder(ItemHolder itemHolder, int i) {
        ...
        if (getItemViewType(i) == 0) {
            ...
            ImageLoader.getInstance().displayImage(AppUtils.getAlbumArtUri(localItem.albumId).toString(),
                    ...
                    public boolean onMenuItemClick(MenuItem item) {
                        switch (item.getItemId()) {
                            case R.id.popup_song_play:
                                MusicPlayer.playAll(mContext, songIDs, position + 1, -1, AppUtils.IdType.NA, false);
                                break;
                            ...
                            case R.id.popup_song_share:
                                AppUtils.shareTrack(mContext, arraylist.get(position + 1).id);
                                break;
                            case R.id.popup_song_delete:
                                long[] deleteIds = {arraylist.get(position + 1).id};
                                AppUtils.showDeleteDialog(mContext,arraylist.get(position + 1).title, deleteIds, ArtistSongAdapter.this, position + 1);
                                break;
                        }
                        return false;
                    }
